...
export default function OffersPage(){
  const {openAction, canAction, contextHolder}=useEnterpriseActions('Offer跟进');
  const color=(s:string)=>s==='已录取'?'green':s==='有条件录取'?'gold':'blue';
  const columns=[{title:'学校',dataIndex:'school'},{title:'专业',dataIndex:'major'},{title:'Offer状态',dataIndex:'status',render:(v:string)=><Tag color={color(v)}>{v}</Tag>},{title:'奖学金',dataIndex:'scholarship'},{title:'结果时间',dataIndex:'resultAt'},{title:'备注',dataIndex:'remark'},{title:'操作',render:(_:unknown,r:any)=> canAction('view') ? <Button type='link' onClick={()=>openAction('view',r)}>查看详情</Button> : null}];
  return <>{contextHolder}
    ...
    <DataTable rowKey='id' columns={columns} dataSource={scopeApplications(offers)} pagination={{total:scopeApplications(offers).length}}/>
  </>
}
