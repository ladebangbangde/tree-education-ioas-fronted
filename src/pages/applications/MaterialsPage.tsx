...
export default function MaterialsPage(){
  const {openAction, contextHolder}=useEnterpriseActions('材料审核');
  const columns=[{title:'材料名称',dataIndex:'name'},{title:'状态',dataIndex:'status',render:(v:string)=><StatusTag status={v}/>},{title:'截止时间',dataIndex:'deadline'},{title:'负责人',dataIndex:'owner'},{title:'备注',dataIndex:'remark'},{title:'完成',render:()=> <Checkbox/>},{title:'操作',render:(_:unknown,r:any)=> <Space><Button type='link' onClick={()=>openAction('file',r)}>材料详情</Button><Button type='link' onClick={()=>openAction('log',r)}>日志</Button></Space>}];
