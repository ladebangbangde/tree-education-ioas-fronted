...
export default function LeadsCenterPage(){
  const [activeTab, setActiveTab] = useState<LeadTab>('unassigned');
  ...
  const loadLeads = useCallback(async () => {
    setLoading(true);
    try {
      const page = await leadsApi.list({ tab: activeTab, keyword: keyword || undefined, relatedPackageId, operatorId, pageNum: 1, pageSize: 200 });
      setData(page.records.map(adaptLead));
  ...
  const columns = [
    { title: '线索编号', dataIndex: 'leadNo' }, { title: '姓名', dataIndex: 'studentName' }, { title: '手机', dataIndex: 'phone' }, { title: '微信', dataIndex: 'wechat' },
    ...
    { title: '操作', fixed: 'right' as const, render: (_: unknown, r: Lead) => <Button type='link' onClick={() => openDetail(r)}>详情</Button> }
  ];
