...
import { applicationFlowsApi } from '@/api/applicationFlows';
...
export default function StudentsListPage() {
  ...
  const [flowLoadingId, setFlowLoadingId] = useState<string | number>();
  ...
  const openApplicationFlow = async (record: StudentRow) => {
    setFlowLoadingId(record.id);
    try {
      try {
        const existing = await applicationFlowsApi.detailByStudent(record.id);
        nav(`/applications/detail/${existing.id}`);
      } catch {
        const created = await applicationFlowsApi.start(record.id);
        message.success('申请流程已创建');
        nav(`/applications/detail/${created.id}`);
      }
    } finally {
      setFlowLoadingId(undefined);
    }
  };

  const columns = [
    { title: '客户编号', dataIndex: 'studentNo', render: (v: string, r: StudentRow) => v || r.id },
    { title: '客户姓名', dataIndex: 'studentName', render: (v: string) => v || '-' },
    ...
    { title: '生成时间', dataIndex: 'createdAt', render: fmt },
    {
      ...
      render: (_: unknown, r: StudentRow) => <Space>
        ...
        <Button type='link' loading={flowLoadingId === r.id} onClick={() => openApplicationFlow(r)}>申请流程</Button>
  ...
  return <>
    ...
    <Modal title='修改客户档案' open={Boolean(editing)} onCancel={() => setEditing(undefined)} onOk={submitEdit} okText='保存' cancelText='取消'>
      <Form form={editForm} layout='vertical'>
        <Form.Item label='客户姓名' name='studentName' rules={[{ required: true, message: '请输入客户姓名' }]}><Input /></Form.Item>
