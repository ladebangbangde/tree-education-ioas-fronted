...
function normalizeConfirmedContent(content: DataOpsContent, topicId: number, contentType: DataOpsContentType, title: string): DataOpsContent {
  return {
    ...content,
    platform_topic_id: contentTopicId(content) || topicId,
    platformTopicId: contentTopicId(content) || topicId,
...
export default function OperationDataPage() {
  ...
  useEffect(() => {
    accountForm.setFieldsValue({ accountName: topicAccountName(selectedTopic), platformUserId: topicPlatformUserId(selectedTopic) });
  }, [selectedTopicId, selectedTopic, accountForm]);

  useEffect(() => {
    if (!selectedTopicId) return;
    const firstConfirmed = contents.find(content => contentTopicId(content) === selectedTopicId);
    const remembered = contentContentType(firstConfirmed) || topicContentType(selectedTopic);
    if (remembered) setSelectedContentType(remembered);
  }, [selectedTopicId, selectedTopic, contents]);
  ...
  const createPackage = async () => {
    ...
    try {
      ...
      const created = await dataOpsApi.createPackage({
        topicDate: v.topicDate || today,
        operatorUserIds: v.operatorUserIds.map(Number),
        mediaUserIds: v.mediaUserIds.map(Number)
      });
  ...
  const deleteCurrentPackage = async () => {
    ...
    try {
      ...
      message.success('主题包及内部关联数据已删除');
      setPackages(rows => rows.filter(item => item.id !== selectedPackage.id));
  ...
  const createPlatformTopic = async () => {
    ...
    try {
      ...
      await dataOpsApi.createPlatformTopic(selectedPackage.id, {
  ...
  const openConfirmContent = (topic: DataOpsPlatformTopic) => {
    ...
    const existing = contents.find(content => contentTopicId(content) === topic.id && contentContentType(content) === selectedContentType)
      || contents.find(content => contentTopicId(content) === topic.id);
    const rememberedType = contentContentType(existing) || topicContentType(topic) || selectedContentType;
    setSelectedContentType(rememberedType);
    setActiveTopic(topic);
    setSelectedTopicId(topic.id);
    contentForm.setFieldsValue({
      contentTitle: contentTitle(existing) || '',
      contentType: rememberedType,
      contentDate: today,
      contentSummary: pick<string>(existing, 'content_summary', 'contentSummary') || ''
    });
    setContentOpen(true);
  };
  ...
  const confirmContent = async () => {
    ...
    try {
      ...
      const created = await dataOpsApi.confirmContent(activeTopic.id, {
        contentTitle: v.contentTitle,
        contentSummary: v.contentSummary,
        contentDate: v.contentDate || today,
      ...
      if (patchedPackage) {
        setSelectedPackage(patchedPackage);
        setPackages(rows => mergePackage(rows, patchedPackage));
      }
      setSelectedContentType(nextType);
      message.success('人工确认完成，数据页上传已解锁');
  ...
  const confirmAccount = async () => {
    ...
    try {
      ...
      message.success('账号已确认，请继续上传图文/视频页面自动识别');
      await refreshPackageDetail();
  ...
  const uploadContentPageAndUnlock = async (topic: DataOpsPlatformTopic, file: File) => {
    ...
    setSelectedTopicId(topic.id);
    try {
      const recognition: any = await dataRecognitionApi.recognize(file, { platform, scene: 'CONTENT_DETAIL', contentType: 'AUTO' });
      ...
      contentForm.setFieldsValue({
        ...
        contentDate: today,
        contentSummary: `AI识别建议：${contentTypeLabelMap[detected]}；页面类型：${recognition.pageType || recognition.result?.pageType || 'CONTENT_PAGE'}；下一步：人工确认后解锁数据页；requestId=${recognition.requestId || ''}`
      });
      setContentOpen(true);
      message.success(`AI 已识别为${contentTypeLabelMap[detected]}，请人工确认类型和标题`);
    } finally {
  ...
  const uploadDataPage = async (group: DataOpsAssetGroup, file: File) => {
    ...
    if (!selectedContent?.id || !selectedTopicId) return message.warning('请先人工确认图文/视频类型和标题，确认后才可上传数据页');
    ...
    try {
      ...
      message.success('数据页已上传并识别；同组旧图由后端自动覆盖');
    } finally {
  ...
  const preview = (asset?: DataOpsAsset) => asset
    ? <Image src={toPreviewUrl(assetUrl(asset))} width="100%" height={130} style={{ objectFit: 'cover', borderRadius: 6 }} />
    : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="未上传" />;

  const assetExtra = (asset?: DataOpsAsset) => asset
    ? <Space><Tag color={statusColor(assetStatus(asset))}>{assetStatus(asset)}</Tag><Popconfirm title="删除这张图片？" okText="删除" cancelText="取消" onConfirm={() => deleteAsset(asset)}><Button danger size="small" type="text" icon={<DeleteOutlined />} loading={deletingAssetId === asset.id} /></Popconfirm></Space>
    : <Tag>未上传</Tag>;

  const contentTypeSelector = (
    <Card size="small" style={{ borderColor: accountConfirmed ? '#1677ff' : undefined }}>
      <Space direction="vertical" style={{ width: '100%' }} size={12}>
        <Alert
          type={selectedContent ? 'success' : 'warning'}
          showIcon
          message={selectedContent ? `当前已确认：${contentTypeLabelMap[selectedContentType]} / ${contentTitle(selectedContent)}` : '必须先选择图文或视频，并人工确认标题后，才能上传数据页'}
        />
        <Radio.Group
        ...
        >
          {contentTypeOptions.map(item => <Radio.Button key={item.value} value={item.value}>{item.label}</Radio.Button>)}
        </Radio.Group>
        <div style={{ color: '#8c8c8c' }}>{contentTypeOptions.find(item => item.value === selectedContentType)?.desc}</div>
      </Space>
    </Card>
  );
  ...
  return <>
    <PageHeader
      ...
      extra={<Space>
        ...
        <Popconfirm title="确定删除当前主题包？" description="会删除内部账号、内容、图片、指标和已生成数据。" okText="删除" cancelText="取消" onConfirm={deleteCurrentPackage}>
          <Button danger icon={<DeleteOutlined />} disabled={!selectedPackage} loading={deletingPackage}>删除当前主题包</Button>
        </Popconfirm>
    ...
    <Row gutter={[16, 16]}>
      ...
      <Col xs={24} sm={12} lg={6}><Card><Statistic title="当前类型" value={contentTypeLabelMap[selectedContentType]} /></Card></Col>
    ...
    <Row gutter={[16, 16]} className="mt12">
      <Col xs={24} lg={5}>
        <Card title="主题路径">
          <Tree showIcon defaultExpandAll selectedKeys={selectedPackage ? [`platform-${selectedPackage.id}-${selectedPlatform}`] : []} treeData={treeData} onSelect={handleTreeSelect} />
        </Card>
      </Col>
      <Col xs={24} lg={19}>
        <Card
          loading={loading}
          title={selectedPackage ? `${packageTitle(selectedPackage)} / ${platformLabelMap[selectedPlatform]}` : '主题包工作区'}
          extra={<Space><Tag color="blue">{platformLabelMap[selectedPlatform]}</Tag><Button icon={<PlusOutlined />} onClick={() => openTopicModal(selectedPlatform)}>创建平台子主题</Button></Space>}
        >
          {!selectedPackage ? <Empty description="请先创建或选择主题包" /> : <>
            <Table
              ...
              columns={[
                ...
                { title: '操作', width: 430, render: (_, row: DataOpsPlatformTopic) => <Space wrap onClick={event => event.stopPropagation()}>
                  ...
                  <Button onClick={() => { setSelectedTopicId(row.id); accountForm.setFieldsValue({ accountName: topicAccountName(row), platformUserId: topicPlatformUserId(row) }); }}>确认账号</Button>
                </Space> }
              ]}
            />
            ...
            <Card type="inner" className="mt12" title={selectedTopic ? `账号主页确认：${topicName(selectedTopic)}` : '账号主页确认'} extra={selectedTopic ? <Tag color={accountConfirmed ? 'green' : 'gold'}>{accountConfirmed ? '账号已确认' : '必须先确认账号'}</Tag> : null}>
              {selectedTopic ? <Row gutter={[16, 16]}>
                ...
                <Col xs={24} lg={16}>
                  <Form form={accountForm} layout="vertical"><Row gutter={12}><Col xs={24} md={12}><Form.Item name="accountName" label="账号名称" rules={[{ required: true, message: '请确认账号名称' }]}><Input placeholder="OCR识别后可人工修正" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name="platformUserId" label={platformIdLabelMap[selectedPlatform]} rules={[{ required: true, message: '请确认平台账号ID' }]}><Input placeholder="例如抖音号/视频号/小红书号" /></Form.Item></Col></Row><Button type="primary" loading={accountConfirming} onClick={confirmAccount}>确认账号后上传图文/视频页面</Button></Form>
                  <div style={{ marginTop: 12, color: accountConfirmed ? '#52c41a' : '#fa8c16' }}>{accountConfirmed ? `已确认：${topicAccountName(selectedTopic)} / ${topicPlatformUserId(selectedTopic)}。下一步请上传图文或视频页面，AI识别后还需要人工确认。` : '未确认前，图文/视频页面上传、数据页上传全部禁止。'}</div>
                </Col>
              </Row> : <Empty description="请选择一个平台子主题" />}
            </Card>

            <Card type="inner" className="mt12" title="账号下的内容类型" extra={<Tag color={accountConfirmed ? 'green' : 'default'}>{accountConfirmed ? `账号：${topicAccountName(selectedTopic)}` : '先确认账号'}</Tag>}>
              <Row gutter={[16, 16]}>
                <Col xs={24} lg={10}>{contentTypeSelector}</Col>
                <Col xs={24} lg={14}>
                  <Space wrap>
                    <Upload showUploadList={false} beforeUpload={file => { selectedTopic && uploadContentPageAndUnlock(selectedTopic, file); return false; }}><Button type="primary" disabled={!accountConfirmed} loading={uploadingContentPage} icon={<CloudUploadOutlined />}>上传图文/视频页面识别</Button></Upload>
                    <Button disabled={!accountConfirmed} onClick={() => selectedTopic && openConfirmContent(selectedTopic)}>手动确认/修正类型和标题</Button>
                  </Space>
                  <div style={{ marginTop: 12, opacity: 0.72 }}>{selectedContent ? `已人工确认：${contentTypeLabelMap[selectedContentType]} / ${contentTitle(selectedContent)}` : 'AI只给建议，必须人工确认类型和标题后，才允许上传数据页。'}</div>
                </Col>
              </Row>
            </Card>

            <Card type="inner" className="mt12" title="账号 → 图文/视频 → 数据表" extra={topicMetricStatus ? <Space><Tag color={selectedStatus.color}>{selectedStatus.label}</Tag><span>成功 {topicMetricStatus.success || 0}/{topicMetricStatus.total || 0}</span></Space> : null}>
            ...
            <Card type="inner" className="mt12" title={`${contentTypeLabelMap[selectedContentType]}数据页上传`}>
              {selectedTopic ? <Row gutter={[16, 16]}>
                <Col xs={24} lg={8}>
                  <Card size="small" title={`${contentTypeLabelMap[selectedContentType]}内容`} extra={<Tag color={selectedContent ? 'green' : 'gold'}>{selectedContent ? '已人工确认' : '待确认内容'}</Tag>}>
                    <Statistic title="当前标题" value={contentTitle(selectedContent) || '待人工确认'} />
                    <Upload showUploadList={false} beforeUpload={file => { uploadContentPageAndUnlock(selectedTopic, file); return false; }}><Button block className="mt12" disabled={!accountConfirmed} loading={uploadingContentPage} icon={<CloudUploadOutlined />}>上传图文/视频页面识别标题</Button></Upload>
                    <Button block className="mt12" disabled={!accountConfirmed} onClick={() => openConfirmContent(selectedTopic)}>手动修正/确认标题</Button>
                  </Card>
                </Col>
                <Col xs={24} lg={16}>
                  <Space direction="vertical" size={16} style={{ width: '100%' }}>
                    {dataGroups.map(group => {
                      ...
                      return <Card key={group.key} size="small" title={group.title} extra={assetExtra(latest)}>
                        <div style={{ marginBottom: 8, color: '#8c8c8c' }}>{group.desc}</div>
                        {preview(latest)}
                        <Upload showUploadList={false} beforeUpload={file => { uploadDataPage(group.key, file); return false; }}><Button className="mt12" icon={<CloudUploadOutlined />} disabled={!selectedContent?.id} loading={uploadingData}>上传/覆盖 {group.title}</Button></Upload>
                      </Card>;
                    })}
                  </Space>
                </Col>
    ...
    <Modal title="创建主题包" open={packageOpen} onCancel={() => setPackageOpen(false)} onOk={createPackage} confirmLoading={packageSubmitting} destroyOnClose>
    ...
    <Modal title="创建平台子主题" open={topicOpen} onCancel={() => setTopicOpen(false)} onOk={createPlatformTopic} confirmLoading={topicSubmitting} destroyOnClose>
      <Form form={topicForm} layout="vertical">
        ...
        <Form.Item name="subTopicName" label="子主题名称"><Input placeholder="例如：抖音账号主页" /></Form.Item>
      </Form>
    </Modal>

    <Modal title="人工确认图文/视频类型和标题" open={contentOpen} onCancel={() => setContentOpen(false)} onOk={confirmContent} confirmLoading={contentSubmitting} destroyOnClose width={720}>
      <Form form={contentForm} layout="vertical">
        <Alert type="info" showIcon message="这里是关键步骤：确认后才会解锁数据页上传。选错图文/视频会导致指标表不对。" style={{ marginBottom: 16 }} />
        <Form.Item name="contentType" label="内容类型" rules={[{ required: true, message: '请选择图文或视频' }]}>
          <Radio.Group optionType="button" buttonStyle="solid" size="large">
            {contentTypeOptions.map(item => <Radio.Button key={item.value} value={item.value}>{item.label}</Radio.Button>)}
          </Radio.Group>
        </Form.Item>
        <Form.Item name="contentTitle" label="内容标题" rules={[{ required: true, message: '请确认标题' }]}><Input.TextArea rows={3} placeholder="OCR识别后可人工修正" /></Form.Item>
        <Form.Item name="contentDate" label="内容日期"><Input placeholder="YYYY-MM-DD" /></Form.Item>
        <Form.Item name="contentSummary" label="识别说明/备注"><Input.TextArea rows={3} /></Form.Item>
      </Form>
    </Modal>
  </>;
}
