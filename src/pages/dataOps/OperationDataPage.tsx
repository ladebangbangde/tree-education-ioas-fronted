...
export default function OperationDataPage() {
  ...
  const uploadContentPageAndUnlock = async (topic: DataOpsPlatformTopic, file: File) => { if (!topicAccountConfirmed(topic)) return message.warning('请先确认账号'); const platform = topicPlatform(topic) || selectedPlatform; setUploadingContentPage(true); setSelectedTopicId(topic.id); try { const recognition = await dataRecognitionApi.recognize(file, { platform, scene: 'CONTENT_DETAIL', contentType: 'AUTO' }); const detected = normalizeDetectedContentType(recognition.contentType || recognition.result?.contentType); if (!detected) return message.warning('未能判断这是图文还是视频，请换一张更完整的数据截图，或手动确认标题'); const title = recognition.result?.contentTitle || recognition.result?.candidateTitles?.[0] || `${contentTypeLabelMap[detected]}内容-${dayjs().format('HHmmss')}`; setSelectedContentType(detected); await dataOpsApi.confirmContent(topic.id, { contentTitle: title, contentType: detected, contentDate: today, contentSummary: `AI自动识别${contentTypeLabelMap[detected]}页面，requestId=${recognition.requestId || ''}` }); message.success(`已识别为${contentTypeLabelMap[detected]}，并自动确认标题：${title}`); await refreshPackageDetail(); await refreshTopicMetrics(topic.id); } finally { setUploadingContentPage(false); } };
  const uploadDataPage = async (group: DataOpsAssetGroup, file: File) => { if (!accountConfirmed) return message.warning('请先确认账号'); if (!selectedContent?.id) return message.warning('请先上传图文/视频页面，自动识别并确认标题'); setUploadingData(true); try { await dataOpsApi.uploadScreenshots(selectedContent.id, [file], group); message.success('数据图上传成功，后台会异步识别'); await refreshPackageDetail(); await refreshTopicMetrics(selectedTopicId); } finally { setUploadingData(false); } };
  ...
  return <>
    ...
    <Row gutter={[16, 16]} className="mt12"><Col xs={24} lg={5}><Card title="主题路径"><Tree showIcon defaultExpandAll selectedKeys={selectedPackage ? [`platform-${selectedPackage.id}-${selectedPlatform}`] : []} treeData={treeData} onSelect={handleTreeSelect} /></Card></Col><Col xs={24} lg={19}><Card loading={loading} title={selectedPackage ? `${packageTitle(selectedPackage)} / ${platformLabelMap[selectedPlatform]}` : '主题包工作区'} extra={<Space><Tag color="blue">{platformLabelMap[selectedPlatform]}</Tag><Button icon={<PlusOutlined />} onClick={() => openTopicModal(selectedPlatform)}>创建平台子主题</Button></Space>}>{selectedPackage ? <>
      ...
      <Card type="inner" className="mt12" title={`${contentTypeLabelMap[selectedContentType]}数据页上传`}>{selectedTopic ? <Row gutter={[16, 16]}><Col xs={24} lg={8}><Card size="small" title={`${contentTypeLabelMap[selectedContentType]}内容`} extra={<Tag color={selectedContent ? 'green' : 'gold'}>{selectedContent ? '已解锁' : '未上传内容页'}</Tag>}><Statistic title="当前标题" value={contentTitle(selectedContent) || 'null'} /><Upload showUploadList={false} beforeUpload={file => { uploadContentPageAndUnlock(selectedTopic, file); return false; }}><Button block className="mt12" disabled={!accountConfirmed} loading={uploadingContentPage} icon={<CloudUploadOutlined />}>上传图文/视频页面识别标题</Button></Upload><Button block className="mt12" disabled={!accountConfirmed} onClick={() => openConfirmContent(selectedTopic)}>手动修正标题</Button></Card></Col><Col xs={24} lg={16}><Space direction="vertical" size={16} style={{ width: '100%' }}>{dataGroups.map(group => { const groupAssets = dataAssets.filter(asset => assetGroup(asset) === group.key); return <Card key={group.key} size="small" title={group.title} extra={<Tag>{groupAssets.length} 张</Tag>}><div style={{ marginBottom: 12, opacity: 0.65 }}>{group.desc}</div><Upload showUploadList={false} beforeUpload={file => { uploadDataPage(group.key, file); return false; }}><Button disabled={!accountConfirmed || !selectedContent} loading={uploadingData} icon={<CloudUploadOutlined />}>上传{group.title}</Button></Upload><Row gutter={[12, 12]} className="mt12">{groupAssets.map(asset => <Col xs={24} md={8} key={asset.id}><Card size="small" title={assetFileName(asset)} extra={assetExtra(asset)}>{preview(asset)}</Card></Col>)}</Row></Card>; })}</Space></Col></Row> : <Empty description="请选择一个子主题" />}</Card>
    ...
    <Modal title={`确认${contentTypeLabelMap[selectedContentType]}标题`} open={contentOpen} onOk={confirmContent} onCancel={() => setContentOpen(false)} confirmLoading={contentSubmitting} destroyOnClose><Form form={contentForm} layout="vertical" initialValues={{ contentDate: today, contentType: selectedContentType }}><Form.Item name="contentTitle" label={`${contentTypeLabelMap[selectedContentType]}标题`} rules={[{ required: true }]}><Input placeholder="优先由图文/视频页面自动识别，也可以手动修正" /></Form.Item><Form.Item name="contentType" label="内容类型" rules={[{ required: true }]}><Select options={contentTypeOptions} /></Form.Item><Form.Item name="contentSummary" label="内容说明"><Input.TextArea rows={3} /></Form.Item><Form.Item name="contentDate" label="内容日期"><Input /></Form.Item></Form></Modal>
  </>;
}
