...
export default function OperationDataPage() {
  ...
  const openConfirmContent = (topic: DataOpsPlatformTopic) => { if (!topicAccountConfirmed(topic)) return message.warning('请先确认账号，再创建图文/视频'); setActiveTopic(topic); setSelectedTopicId(topic.id); contentForm.setFieldsValue({ contentTitle: contentTitle(selectedContent) || '', contentType: selectedContentType, contentDate: today }); setContentOpen(true); };
  const confirmContent = async () => { if (!activeTopic?.id) return; setContentSubmitting(true); try { const v = await contentForm.validateFields(); await dataOpsApi.confirmContent(activeTopic.id, { contentTitle: v.contentTitle, contentSummary: v.contentSummary, contentDate: v.contentDate || today, contentType: v.contentType || selectedContentType }); message.success('图文/视频标题已确认'); setContentOpen(false); contentForm.resetFields(); await refreshPackageDetail(); await refreshTopicMetrics(activeTopic.id); } finally { setContentSubmitting(false); } };
