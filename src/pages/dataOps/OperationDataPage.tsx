...
function mergeContentIntoPackage(pkg: DataOpsPackage, content: DataOpsContent): DataOpsPackage {
  const rows = pkg.contents || [];
  const id = content.id;
  const nextRows = rows.some(row => row.id === id) ? rows.map(row => row.id === id ? { ...row, ...content } : row) : [content, ...rows];
  return { ...pkg, contents: nextRows };
}
function lockContentIntoPackage(pkg: DataOpsPackage | undefined, content: DataOpsContent) {
  if (!pkg) return undefined;
  return mergeContentIntoPackage(pkg, content);
}
function removeAssetsFromPackage(pkg: DataOpsPackage | undefined, assetsToRemove: DataOpsAsset[]) {
  if (!pkg || !assetsToRemove.length) return pkg;
  const removeIds = new Set(assetsToRemove.map(asset => asset.id));
  return { ...pkg, assets: (pkg.assets || []).filter(asset => !removeIds.has(asset.id)) };
}
...
export default function OperationDataPage() {
  ...
  const confirmContent = async () => { if (!activeTopic?.id) return; setContentSubmitting(true); try { const v = await contentForm.validateFields(); const nextType: DataOpsContentType = v.contentType || selectedContentType; const created = await dataOpsApi.confirmContent(activeTopic.id, { contentTitle: v.contentTitle, contentSummary: v.contentSummary, contentDate: v.contentDate || today, contentType: nextType }); const finalTitle = contentTitle(created) || cleanDisplayText(v.contentTitle); const normalized = normalizeConfirmedContent(created, activeTopic.id, nextType, finalTitle); const basePackage = selectedPackage?.id ? await dataOpsApi.packageDetail(selectedPackage.id) : selectedPackage; const patchedPackage = basePackage ? mergeContentIntoPackage(basePackage, normalized) : undefined; if (patchedPackage) { setSelectedPackage(patchedPackage); setPackages(rows => mergePackage(rows, patchedPackage)); } setSelectedContentType(nextType); message.success('人工确认完成，数据页上传已解锁'); setContentOpen(false); contentForm.resetFields(); await refreshTopicMetrics(activeTopic.id); } finally { setContentSubmitting(false); } };
  ...
  const uploadDataPage = async (group: DataOpsAssetGroup, file: File) => { if (!accountConfirmed) return message.warning('请先确认账号'); if (!selectedContent?.id || !selectedTopicId) return message.warning('请先人工确认图文/视频类型和标题，确认后才可上传数据页'); setUploadingData(true); try { const sameGroupAssets = dataAssets.filter(asset => assetGroup(asset) === group); if (sameGroupAssets.length) { await Promise.all(sameGroupAssets.map(asset => dataOpsApi.deleteAsset(asset.id))); const patchedBeforeUpload = removeAssetsFromPackage(selectedPackage, sameGroupAssets); if (patchedBeforeUpload) { setSelectedPackage(patchedBeforeUpload); setPackages(rows => mergePackage(rows, patchedBeforeUpload)); } } const uploaded = await dataOpsApi.uploadScreenshots(selectedContent.id, [file], group); const lockedTitle = contentTitle(uploaded) || contentTitle(selectedContent); const normalized = normalizeConfirmedContent(uploaded, selectedTopicId, selectedContentType, lockedTitle); const patched = lockContentIntoPackage(selectedPackage, normalized); if (patched) { setSelectedPackage(patched); setPackages(rows => mergePackage(rows, patched)); } try { const result = await dataOpsApi.generateCurrentTopicData(selectedTopicId); if (result.package) { const keepContent = normalizeConfirmedContent(normalized, selectedTopicId, selectedContentType, lockedTitle); const resultPackage = mergeContentIntoPackage(result.package, keepContent); setSelectedPackage(resultPackage); setPackages(rows => mergePackage(rows, resultPackage)); } else { await refreshPackageDetail(); } } catch { await refreshPackageDetail(); message.warning('数据页已上传，但自动识别未完成，请稍后点击当前主题识别任务'); } await refreshTopicMetrics(selectedTopicId); message.success(sameGroupAssets.length ? '已覆盖旧数据页，并已触发当前主题识别' : '数据页已上传，并已触发当前主题识别；请在上方数据表查看结果'); } finally { setUploadingData(false); } };
