...
export default function OperationDataPage() {
  ...
  const openConfirmContent = (topic: DataOpsPlatformTopic) => {
    ...
    const existing = contents.find(content => contentTopicId(content) === topic.id && contentContentType(content) === selectedContentType) || contents.find(content => contentTopicId(content) === topic.id);
    ...
    contentForm.setFieldsValue({ contentTitle: contentTitle(existing) || '', contentType: rememberedType, contentDate: today, contentSummary: pick<string>(existing, 'content_summary', 'contentSummary') || '' });
    setContentOpen(true);
  };
        ...
        <Card loading={loading} title={selectedPackage ? `${packageTitle(selectedPackage)} / ${platformLabelMap[selectedPlatform]}` : '主题包工作区'} extra={<Space><Tag color="blue">{platformLabelMap[selectedPlatform]}</Tag><Button icon={<PlusOutlined />} onClick={() => openTopicModal(selectedPlatform)}>创建平台子主题</Button></Space>}>
          {!selectedPackage ? <Empty description="请先创建或选择主题包" /> : <>
            ...
            <Card type="inner" className="mt12" title="账号下的内容类型" extra={<Tag color={accountConfirmed ? 'green' : 'default'}>{accountConfirmed ? `账号：${topicAccountName(selectedTopic)}` : '先确认账号'}</Tag>}>
              <Row gutter={[16, 16]}><Col xs={24} lg={10}>{contentTypeSelector}</Col><Col xs={24} lg={14}><Space wrap><Upload showUploadList={false} beforeUpload={file => { selectedTopic && uploadContentPageAndUnlock(selectedTopic, file); return false; }}><Button type="primary" disabled={!accountConfirmed} loading={uploadingContentPage} icon={<CloudUploadOutlined />}>上传图文/视频页面识别</Button></Upload><Button disabled={!accountConfirmed} onClick={() => selectedTopic && openConfirmContent(selectedTopic)}>手动确认/修正类型和标题</Button></Space><div style={{ marginTop: 12, opacity: 0.72 }}>{selectedContent ? `已人工确认：${contentTypeLabelMap[selectedContentType]} / ${contentTitle(selectedContent)}` : 'AI只给建议，必须人工确认类型和标题后，才允许上传数据页。'}</div></Col></Row>
            </Card>
            <Card type="inner" className="mt12" title="账号 → 图文/视频 → 数据表" extra={topicMetricStatus ? <Space><Tag color={selectedStatus.color}>{selectedStatus.label}</Tag><span>成功 {topicMetricStatus.success || 0}/{topicMetricStatus.total || 0}</span></Space> : null}><AccountVideoMetricTable rows={topicMetrics} loading={metricLoading} contentType={selectedContentType} contentTitle={contentTitle(selectedContent)} /></Card>
            <Card type="inner" className="mt12" title={`${contentTypeLabelMap[selectedContentType]}数据页上传`}>
              {selectedTopic ? <Row gutter={[16, 16]}><Col xs={24} lg={8}><Card size="small" title={`${contentTypeLabelMap[selectedContentType]}内容`} extra={<Tag color={selectedContent ? 'green' : 'gold'}>{selectedContent ? '已人工确认' : '待确认内容'}</Tag>}><Statistic title="当前标题" value={contentTitle(selectedContent) || '待人工确认'} /><Upload showUploadList={false} beforeUpload={file => { uploadContentPageAndUnlock(selectedTopic, file); return false; }}><Button block className="mt12" disabled={!accountConfirmed} loading={uploadingContentPage} icon={<CloudUploadOutlined />}>上传图文/视频页面识别标题</Button></Upload><Button block className="mt12" disabled={!accountConfirmed} onClick={() => openConfirmContent(selectedTopic)}>手动修正/确认标题</Button></Card></Col><Col xs={24} lg={16}><Space direction="vertical" size={16} style={{ width: '100%' }}>{dataGroups.map(group => { const groupAssets = dataAssets.filter(asset => assetGroup(asset) === group.key && (!selectedContent?.id || assetContentId(asset) === selectedContent.id)); const latest = [...groupAssets].sort((a, b) => Number(b.id) - Number(a.id))[0]; return <Card key={group.key} size="small" title={group.title} extra={assetExtra(latest)}><div style={{ marginBottom: 8, color: '#8c8c8c' }}>{group.desc}</div>{preview(latest)}<Upload showUploadList={false} beforeUpload={file => { uploadDataPage(group.key, file); return false; }}><Button className="mt12" icon={<CloudUploadOutlined />} disabled={!selectedContent?.id} loading={uploadingData}>上传/覆盖 {group.title}</Button></Upload></Card>; })}</Space></Col></Row> : <Empty description="请选择一个平台子主题" />}
            </Card>
          </>}
        </Card>
      </Col>
