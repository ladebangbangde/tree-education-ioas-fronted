...
function topicContentType(topic?: DataOpsPlatformTopic): DataOpsContentType { return pick<DataOpsContentType>(topic, 'content_type', 'contentType') || 'IMAGE_TEXT'; }
...
function contentPlatform(content?: DataOpsContent): PlatformCode | undefined { return pick<PlatformCode>(content, 'platform_code', 'platformCode'); }
...
export default function OperationDataPage() {
  ...
  const refreshTopicMetrics = async (topicId?: number) => {
    ...
    try { const result = await dataOpsApi.topicMetrics(topicId); setTopicMetrics(result.rows || []); setTopicMetricStatus(result.status); }
  ...
  const createTopic = async () => { if (!selectedPackage?.id) return message.warning('请先选择主题包'); setTopicSubmitting(true); try { const v = await topicForm.validateFields(); const created = await dataOpsApi.createPlatformTopic(selectedPackage.id, { platformCode: v.platformCode, contentType: v.contentType || selectedContentType, subTopicName: v.subTopicName }); message.success('平台子主题已创建'); setTopicOpen(false); topicForm.resetFields(); setSelectedPlatform(v.platformCode); setSelectedContentType(v.contentType || selectedContentType); setSelectedTopicId(created.id); await refreshPackageDetail(selectedPackage.id); } finally { setTopicSubmitting(false); } };
  const openTopicModal = (platform: PlatformCode) => { topicForm.setFieldsValue({ platformCode: platform, contentType: selectedContentType }); setSelectedPlatform(platform); setTopicOpen(true); };
  const openConfirmContent = (topic: DataOpsPlatformTopic) => { setActiveTopic(topic); setSelectedTopicId(topic.id); contentForm.setFieldsValue({ contentTitle: contentTitle(selectedContent) || topicName(topic), contentType: topicContentType(topic), contentDate: today }); setContentOpen(true); };
  const confirmContent = async () => { if (!activeTopic?.id) return; setContentSubmitting(true); try { const v = await contentForm.validateFields(); await dataOpsApi.confirmContent(activeTopic.id, { contentTitle: v.contentTitle, contentSummary: v.contentSummary, contentDate: v.contentDate || today, contentType: v.contentType || topicContentType(activeTopic) }); message.success('视频标题已确认'); setContentOpen(false); contentForm.resetFields(); await refreshPackageDetail(); await refreshTopicMetrics(activeTopic.id); } finally { setContentSubmitting(false); } };
  const uploadTitlePage = async (topic: DataOpsPlatformTopic, file: File) => { setUploadingTitle(true); setSelectedTopicId(topic.id); try { const updated = await dataOpsApi.uploadCover(topic.id, file); const assetId = topicCoverAssetId(updated) || topicCoverAssetId(topic); if (assetId) await dataOpsApi.recognizeAsset(assetId, { platform: topicPlatform(topic), scene: 'CONTENT_DETAIL' }); message.success('视频标题页/封面识别完成'); await refreshPackageDetail(); await refreshTopicMetrics(topic.id); } finally { setUploadingTitle(false); } };
  const uploadDataPage = async (group: DataOpsAssetGroup, file: File) => { if (!selectedContent?.id) return message.warning('请先上传或确认视频标题页'); setUploadingData(true); try { await dataOpsApi.uploadScreenshots(selectedContent.id, [file], group); message.success('视频数据图上传成功，后台会异步识别'); await refreshPackageDetail(); await refreshTopicMetrics(selectedTopicId); } finally { setUploadingData(false); } };
  const generateCurrentTopicData = async (topic: DataOpsPlatformTopic) => { setUploadingTitle(true); try { const result = await dataOpsApi.generateCurrentTopicData(topic.id); if (result.package) { setSelectedPackage(result.package); setPackages(rows => mergePackage(rows, result.package!)); } else await refreshPackageDetail(); await refreshTopicMetrics(topic.id); message.success('当前主题识别任务已处理'); } finally { setUploadingTitle(false); } };
  ...
  const recognizeTitlePage = async (row: DataOpsPlatformTopic) => { const assetId = topicCoverAssetId(row); if (!assetId) return; await dataOpsApi.recognizeAsset(assetId, { platform: topicPlatform(row), scene: 'CONTENT_DETAIL' }); await refreshTopicMetrics(row.id); await refreshPackageDetail(); };
  const handleTreeSelect = (keys: Key[]) => { const key = String(keys?.[0] || ''); if (key.startsWith('platform-')) { const [, packageId, platform] = key.split('-'); const pkg = packages.find(item => item.id === Number(packageId)); if (pkg) setSelectedPackage(pkg); setSelectedPlatform(platform as PlatformCode); setSelectedContentType('IMAGE_TEXT'); setSelectedTopicId(undefined); } if (key.startsWith('package-')) { const pkg = packages.find(item => item.id === Number(key.replace('package-', ''))); if (pkg) setSelectedPackage(pkg); setSelectedTopicId(undefined); } };
  ...
  return <>
    ...
    <Row gutter={[16, 16]}><Col xs={24} sm={12} lg={6}><Card><Statistic title="当前子主题" value={selectedPlatformTopics.length} /></Card></Col><Col xs={24} sm={12} lg={6}><Card><Statistic title="当前视频" value={selectedContents.length} /></Card></Col><Col xs={24} sm={12} lg={6}><Card><Statistic title="当前图片" value={selectedTopicAssets.length} /></Card></Col><Col xs={24} sm={12} lg={6}><Card><Statistic title="缺失指标" value={topicMetricStatus?.missing || 0} /></Card></Col></Row>
    <Row gutter={[16, 16]} className="mt12"><Col xs={24} lg={5}><Card title="主题路径"><Tree showIcon defaultExpandAll selectedKeys={selectedPackage ? [`platform-${selectedPackage.id}-${selectedPlatform}`] : []} treeData={treeData} onSelect={handleTreeSelect} /></Card></Col><Col xs={24} lg={19}><Card loading={loading} title={selectedPackage ? `${packageTitle(selectedPackage)} / ${platformLabelMap[selectedPlatform]}` : '主题包工作区'} extra={<Space><Tag color="blue">{platformLabelMap[selectedPlatform]}</Tag><Button icon={<PlusOutlined />} onClick={() => openTopicModal(selectedPlatform)}>创建子主题</Button></Space>}>{selectedPackage ? <>
      ...
      <Table rowKey="id" pagination={{ pageSize: 5 }} dataSource={selectedPlatformTopics as any[]} onRow={record => ({ onClick: () => setSelectedTopicId(record.id) })} columns={[{ title: '平台', width: 90, render: (_, row: any) => platformLabelMap[pick(row, 'platform_code', 'platformCode')] }, { title: '类型', width: 90, render: (_, row: any) => <Tag>{contentTypeLabelMap[topicContentType(row)]}</Tag> }, { title: '子主题', render: (_, row: any) => pick(row, 'sub_topic_name', 'subTopicName') }, { title: '账号', render: (_, row: any) => pick(row, 'ocr_account_name', 'ocrAccountName') || 'null' }, { title: platformIdLabelMap[selectedPlatform], render: (_, row: any) => pick(row, 'ocr_platform_user_id', 'ocrPlatformUserId') || 'null' }, { title: '状态', render: (_, row: any) => row.id === selectedTopicId ? <Tag color={selectedStatus.color}>{selectedStatus.label}</Tag> : <Tag>{pick(row, 'ocr_status', 'ocrStatus') || 'pending'}</Tag> }, { title: '操作', width: 430, render: (_, row: DataOpsPlatformTopic) => <Space wrap onClick={event => event.stopPropagation()}><Upload showUploadList={false} beforeUpload={file => { uploadTitlePage(row, file); return false; }}><Button loading={uploadingTitle && selectedTopicId === row.id} icon={<CloudUploadOutlined />}>上传视频标题页</Button></Upload><Button icon={<ScanOutlined />} disabled={!topicCoverAssetId(row)} onClick={() => recognizeTitlePage(row)}>识别标题页</Button><Button onClick={() => openConfirmContent(row)}>手动确认视频标题</Button><Button onClick={() => generateCurrentTopicData(row)}>生成数据</Button></Space> }]} />
      <Card type="inner" className="mt12" title={selectedTopic ? `当前子主题：${topicName(selectedTopic)}` : '当前子主题'} extra={selectedTopic ? <Button loading={metricLoading} onClick={() => refreshTopicMetrics(selectedTopic.id)}>刷新</Button> : null}>{selectedTopic ? <Row gutter={[12, 12]}><Col xs={24} md={8}><Statistic title="账号名称" value={pick<string>(selectedTopic, 'ocr_account_name', 'ocrAccountName') || 'null'} /></Col><Col xs={24} md={8}><Statistic title={platformIdLabelMap[selectedPlatform]} value={pick<string>(selectedTopic, 'ocr_platform_user_id', 'ocrPlatformUserId') || 'null'} /></Col><Col xs={24} md={8}><Statistic title="当前视频标题" value={contentTitle(selectedContent) || pick<string>(selectedTopic, 'ocr_content_title', 'ocrContentTitle') || 'null'} /></Col></Row> : <Empty description="请选择一个子主题" />}</Card>
      <Card type="inner" className="mt12" title="账号 → 视频 → 视频数据" extra={topicMetricStatus ? <Space><Tag color={selectedStatus.color}>{selectedStatus.label}</Tag><span>成功 {topicMetricStatus.success || 0}/{topicMetricStatus.total || 0}</span></Space> : null}><AccountVideoMetricTable rows={topicMetrics} loading={metricLoading} /></Card>
      <Card type="inner" className="mt12" title="视频标题页与数据图上传">{selectedTopic ? <Row gutter={[16, 16]}><Col xs={24} lg={8}><Card size="small" title="视频标题页 / 封面图" extra={titleAsset ? <Tag color={statusColor(assetStatus(titleAsset))}>{assetStatus(titleAsset)}</Tag> : <Tag>未上传</Tag>}>{preview(titleAsset)}<Button block className="mt12" onClick={() => openConfirmContent(selectedTopic)}>手动确认视频标题</Button></Card></Col><Col xs={24} lg={16}><Space direction="vertical" size={16} style={{ width: '100%' }}>{dataGroups.map(group => { const groupAssets = dataAssets.filter(asset => assetGroup(asset) === group.key); return <Card key={group.key} size="small" title={group.title} extra={<Tag>{groupAssets.length} 张</Tag>}><div style={{ marginBottom: 12, opacity: 0.65 }}>{group.desc}</div><Upload showUploadList={false} beforeUpload={file => { uploadDataPage(group.key, file); return false; }}><Button loading={uploadingData} icon={<CloudUploadOutlined />}>上传{group.title}</Button></Upload><Row gutter={[12, 12]} className="mt12">{groupAssets.map(asset => <Col xs={24} md={8} key={asset.id}><Card size="small" title={assetFileName(asset)} extra={<Tag color={statusColor(assetStatus(asset))}>{assetStatus(asset)}</Tag>}>{preview(asset)}</Card></Col>)}</Row></Card>; })}</Space></Col></Row> : <Empty description="请选择一个子主题" />}</Card>
