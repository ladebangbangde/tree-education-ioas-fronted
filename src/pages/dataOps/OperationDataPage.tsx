...
export default function OperationDataPage() {
  ...
  const loadPackages = async () => {
    ...
    try {
      ...
      const hydrated = await Promise.all((rows || []).map(async row => { try { return await dataOpsApi.packageDetail(row.id); } catch { return row; } }));
  ...
  useEffect(() => { if (!activeContents.length) { setSelectedContentId(undefined); return; } if (!selectedContentId || !activeContents.some(content => content.id === selectedContentId)) setSelectedContentId(activeContents[0].id); }, [activeContents, selectedContentId]);
  useEffect(() => { accountForm.setFieldsValue({ accountName: topicAccountName(selectedTopic), platformUserId: topicPlatformUserId(selectedTopic), contentType: topicContentType(selectedTopic) || selectedContentType }); }, [selectedTopicId, selectedTopic, selectedContentType, accountForm]);
  ...
  const createPlatformTopic = async () => { if (!selectedPackage?.id) return; setTopicSubmitting(true); try { const v = await topicForm.validateFields(); await dataOpsApi.createPlatformTopic(selectedPackage.id, { platformCode: v.platformCode, subTopicName: v.subTopicName }); message.success('平台子主题已创建，请上传账号主页并确认账号类型'); setTopicOpen(false); topicForm.resetFields(); await refreshPackageDetail(); } finally { setTopicSubmitting(false); } };
  ...
  const uploadAccountProfile = async (topic: DataOpsPlatformTopic, file: File) => { setUploadingAccount(true); setSelectedTopicId(topic.id); try { const updated = await dataOpsApi.uploadCover(topic.id, file); const assetId = topicCoverAssetId(updated) || topicCoverAssetId(topic); if (assetId) await dataOpsApi.recognizeAsset(assetId, { platform: topicPlatform(topic), scene: 'ACCOUNT_OVERVIEW' }); message.success('账号主页识别完成，请确认账号名称、账号ID和账号类型'); await refreshPackageDetail(); await refreshTopicMetrics(topic.id); } finally { setUploadingAccount(false); } };
  const confirmAccount = async () => { if (!selectedTopic?.id) return; const v = await accountForm.validateFields(); const contentType: DataOpsContentType = v.contentType || 'IMAGE_TEXT'; setAccountConfirming(true); try { await dataOpsApi.confirmAccount(selectedTopic.id, { accountName: v.accountName, platformUserId: v.platformUserId, contentType }); setSelectedContentType(contentType); message.success(`账号已确认，后续只能上传${contentTypeLabelMap[contentType]}内容`); await refreshPackageDetail(); await refreshTopicMetrics(selectedTopic.id); } finally { setAccountConfirming(false); } };
  const changeAccountContentType = async () => { if (!selectedTopic?.id) return; const v = await typeChangeForm.validateFields(); const contentType: DataOpsContentType = v.contentType; setContentTypeChanging(true); try { await dataOpsApi.updateTopicContentType(selectedTopic.id, { contentType }); setSelectedContentType(contentType); setSelectedContentId(undefined); setTypeChangeOpen(false); message.success(`账号内容类型已切换为${contentTypeLabelMap[contentType]}，后续上传将按该类型处理`); await refreshPackageDetail(); await refreshTopicMetrics(selectedTopic.id); } finally { setContentTypeChanging(false); } };
  const openTypeChangeModal = () => { typeChangeForm.setFieldsValue({ contentType: effectiveContentType }); setTypeChangeOpen(true); };
  ...
  const renderContentCard = (content: DataOpsContent) => { const active = selectedContent?.id === content.id; const type = contentContentType(content); return <Card key={content.id} size="small" hoverable onClick={() => { setSelectedContentId(content.id); setSelectedContentType(type); }} style={{ borderColor: active ? '#1677ff' : undefined, background: active ? 'rgba(22,119,255,0.08)' : undefined }} title={<Space><Tag color={contentTypeColorMap[type]}>{contentTypeLabelMap[type]}</Tag><span>{contentTitle(content) || `未命名内容 #${content.id}`}</span></Space>} extra={<Space><Tag color={active ? 'processing' : 'default'}>{active ? '当前目标' : `#${content.id}`}</Tag><Button size="small" type="link" onClick={event => { event.stopPropagation(); selectedTopic && openConfirmContent(selectedTopic, content); }}>编辑</Button></Space>}><div style={{ color: '#8c8c8c' }}>{pick<string>(content, 'content_summary', 'contentSummary') || '暂无备注'}</div></Card>; };
  const contentTypeSelector = <Card size="small" style={{ borderColor: accountConfirmed ? '#1677ff' : undefined }}><Space direction="vertical" style={{ width: '100%' }} size={12}><Alert type={accountConfirmed ? 'success' : 'warning'} showIcon message={accountConfirmed ? `账号已锁定为：${contentTypeLabelMap[effectiveContentType]}` : '请先确认账号属于图文还是视频'} description={accountConfirmed ? '后续内容上传只按该类型处理。需要切换时，点击“手动更改类型”。' : '确认后另一种类型会被锁住，避免同一个账号下图文和视频混用。'} /><Tag color={contentTypeColorMap[effectiveContentType]} style={{ width: 'fit-content' }}>{contentTypeLabelMap[effectiveContentType]}</Tag><div style={{ color: '#8c8c8c' }}>{contentTypeOptions.find(item => item.value === effectiveContentType)?.desc}</div><Button disabled={!accountConfirmed} onClick={openTypeChangeModal}>手动更改类型</Button></Space></Card>;
        ...
        <Card loading={loading} title={selectedPackage ? `${packageTitle(selectedPackage)} / ${platformLabelMap[selectedPlatform]}` : '主题包工作区'} extra={<Space><Tag color="blue">{platformLabelMap[selectedPlatform]}</Tag><Button icon={<PlusOutlined />} onClick={() => openTopicModal(selectedPlatform)}>创建平台子主题</Button></Space>}>
          {!selectedPackage ? <Empty description="请先创建或选择主题包" /> : <>
            <Table rowKey="id" pagination={{ pageSize: 5 }} dataSource={selectedPlatformTopics as any[]} onRow={record => ({ onClick: () => { setSelectedTopicId(record.id); setSelectedContentId(undefined); } })} columns={[
              ...
              { title: '账号类型', render: (_, row: DataOpsPlatformTopic) => { const type = topicContentType(row); return topicAccountConfirmed(row) && type ? <Tag color={contentTypeColorMap[type]}>{contentTypeLabelMap[type]}</Tag> : <Tag color="gold">待确认</Tag>; } },
              ...
              { title: '操作', width: 480, render: (_, row: DataOpsPlatformTopic) => <Space wrap onClick={event => event.stopPropagation()}><Upload showUploadList={false} beforeUpload={file => { uploadAccountProfile(row, file); return false; }}><Button loading={uploadingAccount && selectedTopicId === row.id} icon={<CloudUploadOutlined />}>上传账号主页</Button></Upload><Button icon={<ScanOutlined />} disabled={!topicCoverAssetId(row)} onClick={() => recognizeAccountProfile(row)}>识别账号</Button><Button onClick={() => { setSelectedTopicId(row.id); setSelectedContentId(undefined); accountForm.setFieldsValue({ accountName: topicAccountName(row), platformUserId: topicPlatformUserId(row), contentType: topicContentType(row) || 'IMAGE_TEXT' }); }}>确认账号/类型</Button>{topicAccountConfirmed(row) ? <Button onClick={() => { setSelectedTopicId(row.id); typeChangeForm.setFieldsValue({ contentType: topicContentType(row) || 'IMAGE_TEXT' }); setTypeChangeOpen(true); }}>手动更改类型</Button> : null}</Space> }
            ]} />
            <Card type="inner" className="mt12" title={selectedTopic ? `账号主页确认：${topicName(selectedTopic)}` : '账号主页确认'} extra={selectedTopic ? <Space><Tag color={accountConfirmed ? 'green' : 'gold'}>{accountConfirmed ? '账号已确认' : '必须先确认账号'}</Tag>{accountConfirmed ? <Tag color={contentTypeColorMap[effectiveContentType]}>{contentTypeLabelMap[effectiveContentType]}</Tag> : null}</Space> : null}>
              {selectedTopic ? <Row gutter={[16, 16]}><Col xs={24} lg={8}><Card size="small" title="账号主页截图" extra={assetExtra(accountProfileAsset)}>{preview(accountProfileAsset)}<Upload showUploadList={false} beforeUpload={file => { uploadAccountProfile(selectedTopic, file); return false; }}><Button block className="mt12" icon={<CloudUploadOutlined />} loading={uploadingAccount}>上传/替换账号主页</Button></Upload></Card></Col><Col xs={24} lg={16}><Form form={accountForm} layout="vertical"><Row gutter={12}><Col xs={24} md={12}><Form.Item name="accountName" label="账号名称" rules={[{ required: true, message: '请确认账号名称' }]}><Input placeholder="OCR识别后可人工修正" /></Form.Item></Col><Col xs={24} md={12}><Form.Item name="platformUserId" label={platformIdLabelMap[selectedPlatform]} rules={[{ required: true, message: '请确认平台账号ID' }]}><Input placeholder="例如抖音号/视频号/小红书号" /></Form.Item></Col></Row><Form.Item name="contentType" label="确认账号类型" rules={[{ required: true, message: '请选择图文或视频' }]}><Radio.Group optionType="button" buttonStyle="solid" size="large" disabled={accountConfirmed}>{contentTypeOptions.map(item => <Radio.Button key={item.value} value={item.value}>{item.label}</Radio.Button>)}</Radio.Group></Form.Item><Space><Button type="primary" loading={accountConfirming} onClick={confirmAccount}>{accountConfirmed ? '重新确认账号信息' : '确认账号和类型'}</Button>{accountConfirmed ? <Button onClick={openTypeChangeModal}>手动更改类型</Button> : null}</Space></Form><div style={{ marginTop: 12, color: accountConfirmed ? '#52c41a' : '#fa8c16' }}>{accountConfirmed ? `已确认：${topicAccountName(selectedTopic)} / ${topicPlatformUserId(selectedTopic)}，账号类型：${contentTypeLabelMap[effectiveContentType]}。` : '未确认前，内容页面上传、数据页上传全部禁止。确认后只允许上传对应类型内容。'}</div></Col></Row> : <Empty description="请选择一个平台子主题" />}
            </Card>
            <Card type="inner" className="mt12" title="账号下的内容" extra={<Space><Tag color={accountConfirmed ? 'green' : 'default'}>{accountConfirmed ? `账号：${topicAccountName(selectedTopic)}` : '先确认账号'}</Tag>{accountConfirmed ? <Tag color={contentTypeColorMap[effectiveContentType]}>{contentTypeLabelMap[effectiveContentType]}</Tag> : null}</Space>}>
              <Row gutter={[16, 16]}><Col xs={24} lg={8}>{contentTypeSelector}</Col><Col xs={24} lg={16}><Space wrap><Upload showUploadList={false} beforeUpload={file => { selectedTopic && uploadContentPageAndUnlock(selectedTopic, file); return false; }}><Button type="primary" disabled={!accountConfirmed} loading={uploadingContentPage} icon={<CloudUploadOutlined />}>上传{contentTypeLabelMap[effectiveContentType]}页面识别并新增</Button></Upload><Button disabled={!accountConfirmed} onClick={() => selectedTopic && openConfirmContent(selectedTopic)}>手动新增{contentTypeLabelMap[effectiveContentType]}内容</Button><Button disabled={!selectedTopic || !selectedContent} onClick={() => selectedTopic && selectedContent && openConfirmContent(selectedTopic, selectedContent)}>编辑当前内容</Button></Space><div style={{ marginTop: 12, opacity: 0.72 }}>当前账号已锁定为{contentTypeLabelMap[effectiveContentType]}。点击某条内容后，下面数据页1/2/3会绑定到当前选中内容。</div><Card size="small" title={`${contentTypeLabelMap[effectiveContentType]}内容（${activeContents.length}）`} style={{ marginTop: 12 }}>{activeContents.length ? <Space direction="vertical" style={{ width: '100%' }}>{activeContents.map(renderContentCard)}</Space> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={`暂无${contentTypeLabelMap[effectiveContentType]}内容`} />}</Card></Col></Row>
            </Card>
            ...
            <Card type="inner" className="mt12" title={`${contentTypeLabelMap[effectiveContentType]}数据页上传`}>
              {selectedTopic ? <Row gutter={[16, 16]}><Col xs={24} lg={8}><Card size="small" title="当前上传目标" extra={<Tag color={selectedContent ? 'green' : 'gold'}>{selectedContent ? '已选中内容' : '待选择内容'}</Tag>}><Statistic title="当前标题" value={contentTitle(selectedContent) || '请先新增/选择内容'} /><Upload showUploadList={false} beforeUpload={file => { uploadContentPageAndUnlock(selectedTopic, file); return false; }}><Button block className="mt12" disabled={!accountConfirmed} loading={uploadingContentPage} icon={<CloudUploadOutlined />}>新增{contentTypeLabelMap[effectiveContentType]}内容：上传页面识别</Button></Upload><Button block className="mt12" disabled={!accountConfirmed} onClick={() => openConfirmContent(selectedTopic)}>手动新增{contentTypeLabelMap[effectiveContentType]}内容</Button></Card></Col><Col xs={24} lg={16}><Space direction="vertical" size={16} style={{ width: '100%' }}>{dataGroups.map(group => { const groupAssets = dataAssets.filter(asset => assetGroup(asset) === group.key && selectedContent?.id && assetContentId(asset) === selectedContent.id); const latest = [...groupAssets].sort((a, b) => Number(b.id) - Number(a.id))[0]; return <Card key={group.key} size="small" title={group.title} extra={assetExtra(latest)}><div style={{ marginBottom: 8, color: '#8c8c8c' }}>{group.desc}</div>{preview(latest)}<Upload showUploadList={false} beforeUpload={file => { uploadDataPage(group.key, file); return false; }}><Button className="mt12" icon={<CloudUploadOutlined />} disabled={!selectedContent?.id} loading={uploadingData}>上传到当前{contentTypeLabelMap[effectiveContentType]}内容：{group.title}</Button></Upload></Card>; })}</Space></Col></Row> : <Empty description="请选择一个平台子主题" />}
            </Card>
          </>}
        </Card>
      </Col>
