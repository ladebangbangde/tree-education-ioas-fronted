...
const appendUploadFiles = (formData: FormData, field: 'scripts' | 'videos' | 'images', files?: UploadFile[]) => (files || []).forEach(file => { const raw = file.originFileObj as File | undefined; if (raw) formData.append(field, raw); });
...
export default function MediaResourceCenterPage(){
  ...
  const uploadFilesToPackage = async (values: any) => {
    const formData = new FormData(); appendUploadFiles(formData, 'scripts', values.script?.fileList); appendUploadFiles(formData, 'videos', values.video?.fileList); appendUploadFiles(formData, 'images', values.image?.fileList);
    if ([...formData.keys()].length === 0) { message.warning('请至少上传一类素材文件'); return; }
    setSubmitLoading(true); try { await contentPackagesApi.uploadFiles(values.packageId, formData); message.success('文件已上传'); setUploadOpen(false); uploadForm.resetFields(); loadResources(); } finally { setSubmitLoading(false); }
  };
