...
import { inferUploadTaskFileType, type UploadTaskFileType } from '@/api/uploadTasks';
...
export default function MediaResourceCenterPage(){
  ...
  useEffect(() => uploadManager.subscribe(items => {
    const hasRecentSuccess = items.some(item => item.status === 'success' && Date.now() - item.updatedAt < 5000);
    if (hasRecentSuccess) loadResources().catch(() => undefined);
  }), [loadResources]);
  ...
  const uploadFilesToPackage = async (values: any) => {
    ...
    uploadManager.enqueueMany(packageId, uploadItems);
    message.success(`已加入全局上传队列：${uploadItems.length} 个文件，可切换菜单继续上传`);
  };
  ...
  return <>
    <PageHeader title='媒体资源中心' extra={<Space className='resource-toolbar' size={12}><Input.Search allowClear placeholder='搜索主题 / 运营' onSearch={setQuery} style={{ width: 240 }} /><Select allowClear placeholder='筛选运营' value={operatorId} onChange={setOperatorId} style={{ width: 180 }} options={operators.map(op => ({ value: op.id, label: op.name }))} />{canUseButton(role, 'upload') && <Button icon={<UploadOutlined />} onClick={() => openUpload()}>上传文件</Button>}{canUseButton(role, 'upload') && <Button onClick={() => navigate('/tasks')}>查看任务中心</Button>}{canUseButton(role, 'createPackage') && <Button type='primary' icon={<PlusOutlined />} onClick={() => setCreateOpen(true)}>新建主题包</Button>}</Space>} />
